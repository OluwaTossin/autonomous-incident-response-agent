...
export async function POST(request: NextRequest, context: { params: Promise<{ approvalId: string }> }) {
  ...
  return withBffSession(request, true, (session) => runtime().api.approveApproval(session.accessToken, body.organizationId, body.workspaceId, approvalId, body.reason));
}
