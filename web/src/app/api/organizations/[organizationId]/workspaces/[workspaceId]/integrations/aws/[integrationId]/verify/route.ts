...
export async function POST(request: NextRequest, context: { params: Promise<{ organizationId: string; workspaceId: string; integrationId: string }> }) {
  ...
  return withBffSession(request, true, (session) => runtime().api.verifyAwsIntegration(session.accessToken, organizationId, workspaceId, integrationId, body.expected_version!));
}
