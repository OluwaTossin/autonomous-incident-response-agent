...
variable "enable_triage_ui_cloudfront" {
  type        = bool
  default     = true
  description = "If true (default), private S3 + CloudFront (HTTPS) for the operator UI — recommended for Version 2 demos. Set false for S3 static website (HTTP) only if your account cannot create CloudFront distributions."
}

# --- Phase 13 — CloudWatch (module monitoring) ---
