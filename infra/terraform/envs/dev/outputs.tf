...
output "ssm_container_secrets" {
  description = "Create each parameter_name as SSM SecureString; ECS maps it to environment_variable (valueFrom ARN)"
  value = [
...
output "openai_ssm_parameter_arn" {
  description = "OpenAI key ARN when openai_api_key_ssm_parameter is set (legacy output)"
