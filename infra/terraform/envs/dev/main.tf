...
locals {
  ...
  # Shorthand for OpenAI; merged with ssm_secrets (duplicate env_name fails check below).
...
# ECS pins the digest of :latest so each apply picks up whatever the push script last pushed
# (immutable at runtime; no manual api_image_tag in tfvars).
